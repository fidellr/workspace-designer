...
export default function LeftPanel({ products }: { products: Product[] }) {
  const [leftTab, setLeftTab] = useState<"chairs" | "desks" | "accessories">(
    "chairs"
  );
  const [isMinimized, setIsMinimized] = useState(false);

  const setMainItem = useWorkspaceStore((state) => state.setMainItem);
  ...
  const handleSelectItem = (item: Product) => {
    if (item.category === "accessories") addAccessory(item);
    else setMainItem(item);
  };
  ...
  return (
    <aside className="absolute left-4 top-4 w-80 bg-white border border-slate-300 rounded-lg shadow-sm z-40 overflow-hidden flex flex-col transition-all">
